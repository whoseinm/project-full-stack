...
<div class="container">
    <div class="card">
        <h5 class="card-header spaceB">Təhsillərin Siyahısı
            <a href="<?php echo base_url('studyAbroad_create') ?>">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <?php $say = 0;
                        foreach ($get_all_study_abroad as $item) {
                            strrev($say++) ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $title = mb_strimwidth($item['abroad_name'], 0, 30, '...');
                                    echo $title ?>
                                </td>
                                </td>
                                <td>
                                    <?php echo date("d-m-Y", strtotime($item['abroad_add_date'])); ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <?php if ($item["abroad_img"]) { ?>
                                        <img width="70px" height="60px" style="object-fit: cover;"
                                            src="<?php echo base_url('uploads/studyAbroad/' . $item['abroad_img']); ?>" alt="img"></img>
                                    <?php } else { ?>
                                ...
                                <td>
                                    <?php if ($item['abroad_status'] == "Active") { ?>
                                        <span class="badge bg-label-success me-1">
                                            <?php echo $item['abroad_status']; ?>
                                        </span>
                                    <?php } else if ($item['abroad_status'] == "Deactive") { ?>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo base_url("studyAbroad_detail/". $item["abroad_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-info">Detail</button>
                                    </a>
                                    <a href="<?php echo base_url("studyAbroad_edit/". $item["abroad_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-warning">Edit</button>
                                    </a>
                                    <a href="<?php echo base_url("studyAbroad_delete/". $item["abroad_id"]) ; ?>">
                                        <button onclick="return confirm('Xaricdə təhsili silmək istədiyinizə əminsiz?')" type="button"
