...
<div class="container">
    <div class="card">
        <h5 class="card-header spaceB">Vakansiyaların Siyahısı
            <a href="<?php echo base_url('vacancy_create') ?>">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <?php $say = 0;
                        foreach ($get_all_vacancies as $item) {
                            strrev($say++) ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $title = mb_strimwidth($item['vacancy_name'], 0, 30, '...');
                                    echo $title ?>
                                </td>
                                </td>
                                <td>
                                    <?php echo date("d-m-Y", strtotime($item['vacancy_add_date'])); ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <?php if ($item["vacancy_img"]) { ?>
                                        <img width="70px" height="60px" style="object-fit: cover;"
                                            src="<?php echo base_url('uploads/vacancy/' . $item['vacancy_img']); ?>" alt="img"></img>
                                    <?php } else { ?>
                                ...
                                <td>
                                    <?php if ($item['vacancy_status'] == "Active") { ?>
                                        <span class="badge bg-label-success me-1">
                                            <?php echo $item['vacancy_status']; ?>
                                        </span>
                                    <?php } else if ($item['vacancy_status'] == "Deactive") { ?>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo base_url("vacancy_detail/". $item["vacancy_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-info">Detail</button>
                                    </a>
                                    <a href="<?php echo base_url("vacancy_edit/". $item["vacancy_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-warning">Edit</button>
                                    </a>
                                    <a href="<?php echo base_url("vacancy_delete/". $item["vacancy_id"]) ; ?>">
