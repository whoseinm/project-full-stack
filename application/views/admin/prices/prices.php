...
<div class="container">
    <div class="card">
        <h5 class="card-header spaceB">Qiymətlərin Siyahısı
            <a href="<?php echo base_url('prices_create') ?>">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <?php $say = 0;
                        foreach ($get_all_prices as $item) {
                            strrev($say++) ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $title = mb_strimwidth($item['price_name'], 0, 30, '...');
                                    echo $title ?>
                                </td>
                                </td>
                                <td>
                                    <?php echo date("d-m-Y", strtotime($item['price_add_date'])); ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <?php if ($item["price_img"]) { ?>
                                        <img width="70px" height="60px" style="object-fit: cover;"
                                            src="<?php echo base_url('uploads/prices/' . $item['price_img']); ?>" alt="img"></img>
                                    <?php } else { ?>
                                ...
                                <td>
                                    <?php if ($item['price_status'] == "Active") { ?>
                                        <span class="badge bg-label-success me-1">
                                            <?php echo $item['price_status']; ?>
                                        </span>
                                    <?php } else if ($item['price_status'] == "Deactive") { ?>
                                ...
                                <td>
                                    ...
                                    <a href="<?php echo base_url("prices_detail/". $item["price_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-info">Detail</button>
                                    </a>
                                    <a href="<?php echo base_url("prices_edit/". $item["price_id"]) ; ?>">
                                        <button type="button" class="btn btn-sm btn-outline-warning">Edit</button>
                                    </a>
                                    <a href="<?php echo base_url("prices_delete/". $item["price_id"]) ; ?>">
